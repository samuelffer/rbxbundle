...
from .parser import get_properties_node, get_name, get_source, get_value, iter_top_level_items, parse_attributes
from .utils import sanitize_filename, safe_open_csv, safe_write_text, wipe_dir, local_tag, strip_junk_before_roblox, read_text
...
CONTEXT_CLASSES = {
    ...
    "StringValue", "NumberValue", "BoolValue", "IntValue", "ObjectValue",
...
def create_bundle(in_path: Path, *, output_dir: Path, include_context: bool) -> Tuple[Path, Path, List[ScriptRecord]]:
    ...
    def walk(item: ET.Element, parent_path: str, depth: int) -> None:
        ...
        if class_name in SCRIPT_CLASSES:
            ...
            suffix = ".server.lua" if class_name == "Script" else ".client.lua" if class_name == "LocalScript" else ".lua"
            ...
            scripts.append(ScriptRecord(class_name, name, full_path, str(Path("scripts") / rel), len(src)))
