...
from . import __version__ as RBXBUNDLE_VERSION
...
def _manifest_payload(
    *,
    ...
    return {
        "rbxbundle_version": RBXBUNDLE_VERSION,
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "input_filename": input_filename,
        "counts": {
            "scripts_total": len(scripts),
            "modules_total": sum(1 for s in scripts if _script_exec_side(s) == "module"),
            "client_total": sum(1 for s in scripts if _script_exec_side(s) == "client"),
            "server_total": sum(1 for s in scripts if _script_exec_side(s) == "server"),
            "edges_total": len(edges_json),
            "nodes_total": len(nodes_json),
            "warnings_total": len(warnings),
        },
        "sha256": _sha256_for_file(input_path),
    }
...
def generate_summary(
    *,
    ...
    lines += [
        ...
        "6. Use `MANIFEST.json` for bundle provenance and counts.",
        "7. Use `CONTEXT.txt` for RemoteEvent / ValueObject details.",
        "",
